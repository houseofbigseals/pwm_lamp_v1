...
// fully stolen from https://github.com/Anacron-sec/esp32-DHT11/tree/master
// simply read data from dht11 using protocol timings (realized using blocking wait)
...
// Task to read DHT11 data and store it in the queue
void dht11_task(void *pvParameter)
{
    ...
    while (1)
    {
        ...
        // Update the queue with the new reading
        xQueueOverwrite(dht11_queue, &reading);

        // Print the reading for debugging purposes
        ESP_LOGI("dht11", "DHT11 Reading - Status: %d, Temperature: %d, Humidity: %d\n",
               reading.status, reading.temperature, reading.humidity);
